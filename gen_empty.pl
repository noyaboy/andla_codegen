...
# 檔案路徑設定
my $input_filename       = 'input/andla_empty.tmp.v';
my $output_filename      = 'output/andla_empty.v';
    ...
    sub write_exceptio {
        ...
        foreach my $key (
            ...
            if ($key eq 'ldma2' || $key eq 'csr') {
                ...
                print { $self->{outfile} }
                    "output                 rf_${key}_except_trigger;\n"
            }
...
############################################
{
    ...
    sub write_baseaddrselio {
        ...
        foreach my $keys ( keys %{ $self->{seen_dma}  }  ) {
            my $uckeys = uc($keys);
            print {$self->{outfile}} "input [${uckeys}_BASE_ADDR_SELECT_BITWIDTH-           1:0] rf_${keys}_base_addr_select;\n"
        }
...
############################################
{
    ...
    sub write_baseaddrselport {
        ...
        foreach my $keys ( keys %{ $self->{seen_dma}  }  ) {
            my $uckeys = uc($keys);
            print {$self->{outfile}} ",rf_${keys}_base_addr_select\n"
        }
...
assign rf_${keys}_base_addr_select            = ${keys}_base_addr_select_reg;\n\n"
        }
        ...
        unless ($self->_skip()) {
            if ($self->{type} eq 'ro') {
                ...
                if ($self->{register} eq 'sfence') {
                    push @{$self->{io_lines}}, "input\t" . " [1-1:0] rf_" . $self->{item} . "_" . $self->{register} . ";";
                    $self->{seen_items}->{$self->{key}} = 1;
                } else { 
