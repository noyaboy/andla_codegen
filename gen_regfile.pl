    ...
    sub write_port {
        ...
        while (my $line = <$dict_fh>) {
            ...
            if ($line =~ /'Item': '([^']*)', 'Register': '([^']*)'/) {
                ...
                next if ($item eq 'csr' && $register =~ /exram_based_addr/);
    ...
    sub _process {
        ...
        unless ($self->_skip()) {
            if ($self->{type} eq 'ro') {
                ...
                if ($self->{item} eq 'csr' && $self->{register} =~ /exram_based_addr/) {
                    push @{$self->{io_lines}}, "wire\t" . " [" . uc($self->{item}) . "_" . uc($self->{register}) . "_BITWIDTH-1:0] " . $self->{item} . "_" . $self->{register} . ";";
                    $self->{seen_items}->{$self->{key}} = 1;
                } elsif ($self->{register} eq 'sfence') {
...
############################################
{
    ...
    sub _skip {
        ...
        if ($self->{register}) {
            ...
            return 1 if ($self->{register} eq 'exram_addr');
        ...
        unless ($self->_skip()) {
            if ($self->{subregister}) {
                    ...
                    )
                {
                    ...
                    my $assignment = "assign ".$self->{item}."_".$output_register_name." = {" . $self->{item} . "_" . $self->{register} . "_msb_reg, " . $self->{item} . "_" . $self->{register} . "_lsb_reg};";
